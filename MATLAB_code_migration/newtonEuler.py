...
#			
			...
			#angular velocity
				...
				w_i = w_ip1
				## NOTE: w_i must be returned as a 3x1 matrix
				...
				dw_i = dw_ip1
				# NOTE: dw_i must be returned as a 3x1 matrix
...
def angularVel(R,w_i,ddq,z,joint_config):
	if joint_config == 0:
		w_ip1 = numpy.dot(R,w_i)
		w_i = w_ip1
	elif joint_config == 1:
		w_ip1 = numpy.dot(R,w_i)+numpy.dot(ddq,z)
		w_i = w_ip1
	return w_i
	
def angularAccel(R,dw_i,dq,z,joint_config):
	if joint_config == 0:
		dw_ip1 = numpy.dot(R,dw_i)
		dw_i = dw_ip1
	elif joint_config == 1:
		R_dw_dot = numpy.dot(R,dw_i)
		dw_ip1 = R_dw_dot + numpy.cross(R_dw_dot.T,numpy.dot(dq_i,z).T).T + numpy.dot(ddq,z)
		dw_i = dw_ip1
		
	return dw_i
	
def linearAccel(R,dw_i,P,dv_i):
	if joint_config == 0: 
